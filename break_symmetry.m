function new_G = break_symmetry(G, graph_size, break_amount)
    ...
    for i = 1:break_amount   
        ...
        if mod(i,2) == 0
            ...
            new_G.A(numRows+i,numCols+i-2*counter_1-4) = 1;
            %[numRows2,numCols2] = size(new_G.A)
            new_G = grasp_symetrise_unweighted(new_G);
            new_G.layout(numRows+i, 1) = new_G.layout(numRows,1)-1;
            new_G.layout(numRows+i, 2) = new_G.layout(numRows,2) + sqrt(3)*(counter_1)+1;
        elseif mod(i,2) == 1
            counter_2 = counter_2 + 1;
            new_G.A(numRows+i,numCols+i) = 0;
            new_G.A(numRows+i,numCols+i-2*counter_2-4) = 1;
            %[numRows2,numCols2] = size(new_G.A)
            new_G = grasp_symetrise_unweighted(new_G);
            new_G.layout(numRows+i, 1) = new_G.layout(numRows-1,1)-1;
            new_G.layout(numRows+i, 2) = new_G.layout(numRows-1,2) + sqrt(3)*(counter_2)+1;
        end
    end
